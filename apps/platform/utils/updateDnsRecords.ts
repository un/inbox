...
const tracer = getTracer('platform/update-dns-records');
...
export async function updateDnsRecords(
  ...
  return tracer.startActiveSpan('Update DNS Records', async (span) => {
    ...
    span?.setAttributes(flatten({ 'domain.info': domainResponse }));
    ...
    span?.setAttributes(flatten({ 'dns.results': currentDNSRecords }));
    ...
    span?.setAttributes(flatten({ 'dns.status': dnsStatus }));
