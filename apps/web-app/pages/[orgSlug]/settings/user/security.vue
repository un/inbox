<script setup lang="ts">
  ...
  watch(initialUserProfile, (newVal) => {
  ...
  async function save() {
    ...
    const updateAccount = $trpc.auth.security.updateCredentials.useMutation();
  ...
  async function saveNickname(credentialId: Uint8Array) {
    const toast = useToast();
    const isSuccess = await $trpc.auth.passkey.renamePasskey.useLazyQuery({
      cred: credentialId,
  ...
  async function removePasskey(credentialId: Uint8Array) {
    const toast = useToast();
    const isSuccess = await $trpc.auth.passkey.deletePasskey.useLazyQuery({
      cred: credentialId
    });
    if (!isSuccess.data.value.success) {
      toast.add({
  ...
  async function savePassword() {
    ...
    buttonLoading.value = true;
    buttonLabel.value = 'Saving...';
    const checkPassword = $trpc.auth.security.checkPassword.useMutation();
    ...
    const setPassword = $trpc.auth.security.setPassword.useMutation();
  ...
  <div class="h-full w-full flex flex-col items-start gap-8 p-4">
    ...
    <div
      ...
      <div class="flex flex-row">
        <div class="text-1xl basis-1/2 font-display">Change Recovery Email</div>
      ...
      <div class="flex">
        <div class="text-1xl basis-1/2 font-display">Change Password</div>
        <div class="flex basis-1/2 flex-col gap-4">
          <div v-if="isPassword">
            <UnUiInput
          ...
          <div class="gap-4 text-sm font-sans">
            <p class="text-sm font-display">Password</p>
            <span> Can Only be done if Passkeys are on </span>

            <UnUiToggle v-model="isPassword" />
          </div>
          <UnUiButton
            label="Add Password"
            icon="i-ph-key"
      ...
      <hr class="line dark:border-gray-600 border-gray-300 border-t" />
      <div class="flex">
        <div class="text-1xl basis-1/2 font-display">Authentication</div>
        <div class="flex basis-1/2 flex-col gap-4">
          <span class="basis-1/2 text-sm font-display"
          ...
          <div class="gap-4 text-sm font-sans">
            <p class="text-sm font-display">Passkeys</p>
            <div class="flex gap-4">
              <span> Setup a More Secure Way of Authentication </span>
            </div>
          </div>
          <UnUiButton
            label="Add a Passkey"
            ...
            @click="addPasskey()" />
          <hr
            class="line dark:border-gray-600 border-gray-300 w-3/4 border-t" />
          <div v-if="passkeysData">
            <div
              v-for="(passkey, index) in passkeysData"
              :key="index"
              class="flex flex-row gap-2 text-sm font-sans">
              ...
              <div class="flex flex-row items-center gap-4">
                ...
                <UnUiModal v-model="removeModalOpen">
                  ...
                  <div class="flex flex-col">
                    <span class="p-2">
                      Are you Sure You want to remove it? Type
      ...
      <hr class="line dark:border-gray-600 border-gray-300 border-t" />
      <div class="flex flex-row">
        <div class="text-1xl basis-1/2 font-display">Sessions</div>
        <div class="flex basis-1/2 flex-col gap-4">
          <div class="text-1xl basis-1/2 font-display">
            ...
            <span>
              <hr
                class="line dark:border-gray-600 w-3/4 border-gray-300 border-t" />
            </span>
          </div>
          <span class="text-sm basis-1/2 font-display"
            >Sign Out Everywhere
            <br />
            <span
              class="text-sm text-red-600 dark:text-red-300 basis-1/2 font-display"
              >Warning: This signs you out of you're account everywhere</span
            >
          </span>
