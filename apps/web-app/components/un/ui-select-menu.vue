<script lang="ts">
  //* This is fork of the original NuxtUI select menu component - see ui.nuxt.com for the original
  ...
  export default defineComponent({
    ...
    setup(props, { emit, slots }) {
      const { ui, attrs } = useUI('select', props.ui, config, {
        mergeWrapper: true
      });
      ...
      const { emitFormBlur, emitFormChange, inputId, color, size, name } =
        ...
        return (props.options as any[]).filter((option: any) => {
          return (
            ...
            return ['string', 'number'].includes(typeof option)
              ? option.toString().search(new RegExp(query.value, 'i')) !== -1
              : option[searchAttribute] &&
                  option[searchAttribute].search(
...
<template>
  <component
    ...
    <div
      v-if="open"
      ref="container"
      :class="[uiMenu.container, uiMenu.width]">
      <Transition
        appear
        v-bind="uiMenu.transition">
        <component
          ...
          <component
            ...
            <li
              :class="[
                uiMenu.option.base,
                uiMenu.option.rounded,
                uiMenu.option.padding,
                uiMenu.option.size,
                uiMenu.option.color,
                active ? uiMenu.option.active : uiMenu.option.inactive,
                selected && uiMenu.option.selected,
                optionDisabled && uiMenu.option.disabled
              ]">
              <div :class="uiMenu.option.container">
                <slot
                  ...
                  <UIcon
                    ...
                    :name="option.icon"
                    :class="[
                      uiMenu.option.icon.base,
                      active
                        ? uiMenu.option.icon.active
                        : uiMenu.option.icon.inactive,
                      option.iconClass
                    ]"
                    aria-hidden="true" />
                  <UAvatar
                    ...
                    v-bind="{
                      size: uiMenu.option.avatar.size,
                      ...option.avatar
                    }"
              ...
              <span
                v-if="selected"
                :class="[
                  uiMenu.option.selectedIcon.wrapper,
                  uiMenu.option.selectedIcon.padding
                ]">
                <UIcon
                  :name="selectedIcon"
                  :class="uiMenu.option.selectedIcon.base"
                  aria-hidden="true" />
              </span>
            </li>
          </component>
              ...
              <div :class="uiMenu.option.container">
                <slot
                  ...
                  <span class="block truncate"
                    >Create "{{ queryOption[optionAttribute] }}"</span
                  >
                </slot>
              </div>
            </li>
