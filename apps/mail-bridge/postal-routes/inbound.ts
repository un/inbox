  ...
  async (c) => {
    ...
    if (orgId === 0 && mailserverId === 'root') {
      ...
      if (!rootEmailIdentity) {
        ...
        return c.json(
          { error: `Email address ${rcpt_to} doesn't exists` },
          404
        );
      }
      ...
      if (!mailServer || mailServer.orgId !== orgId) {
        ...
        return c.json({ error: 'Mailserver not found' }, 404);
      }
      ...
      return c.json({ error: 'Invalid orgId and mailserverId' }, 400);
    }
    ...
    if (!parsedEmail.from) {
      ...
      return c.json({ error: 'No from address found in the email' }, 400);
    }
    ...
    if (!parsedEmail.to) {
      ...
      return c.json({ error: 'No to address found in the email' }, 400);
    }
    ...
    if (
      ...
      return c.json({ error: 'Missing email attributes' }, 400);
    }
    ...
    if (
      ...
      return c.json(
        {
          error: 'No messageToPlatformObject or messageFromPlatformObject found'
        },
        400
      );
    }
    ...
    if (messageFromPlatformObject[0]?.type === 'contact') {
      ...
      if (!contact) {
        ...
        return c.json({ error: 'No contact found for from address' }, 400);
      }
    ...
    if (!emailIdentityIds.length) {
      ...
      const bounceMessage = `This email was sent to an address that does not exist in our system. Please contact the sender to update their records.`;
      return c.text(bounceMessage, 404);
    }
    ...
    if (!fromAddressPlatformObject) {
      ...
      return c.json({ error: 'No from address platform object found' }, 400);
    }
    ...
    if (inReplyToEmailId) {
      ...
      if (existingMessage) {
        ...
        if (!existingMessage.convoId || convoId === 0) {
          ...
          return c.json(
            { error: 'No convoId found for existing message' },
            400
          );
        }
