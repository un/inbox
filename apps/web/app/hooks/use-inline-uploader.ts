      ...
      createImageUpload({
        ...
        onUpload: async (file) => {
          const presignedData = (await fetch(
            `${env.PUBLIC_STORAGE_URL}/api/attachments/presign?orgShortcode=${orgShortcode}&filename=${sanitizeFilename(file.name)}`,
            {
              method: 'GET',
              credentials: 'include'
            }
          ...
          return new Promise((resolve, reject) => {
            toast.promise(
              uploadTracker({
                ...
                image.src = `${env.PUBLIC_STORAGE_URL}/inline-proxy/${orgShortcode}/${presignedData.publicId}/${sanitizeFilename(file.name)}?type=${encodeURIComponent(file.type)}&size=${file.size}`;
                image.onload = () => {
                  resolve(image.src);
                };
                image.onerror = () => {
