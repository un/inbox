...
import { useOrgScopedRouter, useOrgShortcode } from '@/hooks/use-params';
import { type InferQueryLikeData } from '@trpc/react-query/shared';
import { Separator } from '@/components/shadcn-ui/separator';
import { Button } from '@/components/shadcn-ui/button';
import { Link, useNavigate } from '@remix-run/react';
import { logoutCleanup, platform } from '@/lib/trpc';
      ...
      {spaceData.canSeeSettings && (
        <div className="w-0 overflow-hidden transition-all group-hover:w-8">
          ...
          <Button
            ...
            variant="ghost"
            onClick={() =>
              navigate(`/${orgShortCode}/${spaceData.shortcode}/settings`)
            }>
            <DotsThree />
          </Button>
          {/* </DropdownMenuTrigger>
...
export function OrgMenuContent() {
  ...
  const { mutate: logOut, isPending: loggingOut } = useMutation({
    mutationFn: async () => {
      await fetch(`${env.PUBLIC_PLATFORM_URL}/auth/logout`, {
        method: 'POST',
        credentials: 'include'
      });
      logoutCleanup();
      ...
      <DropdownMenuGroup
        ...
        {orgData?.userOrgs.map(({ org }) => (
          <DropdownMenuItem
            ...
            <div
              onClick={() => {
                navigate(`/${org.shortcode}/personal/convo`);
              }}
              className={
