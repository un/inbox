 ...
 */
function blockRemoteContentInAttributes($: CheerioAPI, replacements: Required<ReplacementOptions>) {
...
function getUrlAttributes(
  ...
  const attrs = $el.attr();
  const potentialAttributes: string[] = TAGS_THAT_HAVE_URL_ATTRIBUTES[tagName] || [];

  return potentialAttributes.filter(Object.prototype.hasOwnProperty.bind(attrs));
}

const IMAGE_ATTRIBUTES = new Set(["background", "icon", "placeholder", "poster", "src", "srcset"]);
