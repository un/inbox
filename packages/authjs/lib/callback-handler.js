import { AccountNotLinkedError, OAuthAccountNotLinked } from "../errors.js";
...
export async function handleLogin(sessionToken, _profile, _account, options) {
    ...
    if (!["email", "oauth", "oidc", "passkey"].includes(_account.type))
    ...
    if (account.type === "email") {
        ...
        if (user) {
            if (userByAccount) {
                // If the account belongs to the user, do nothing
                if (userByAccount.id === user.id) {
                    return { session, user, isNewUser: false };
                    // If the account belongs to another user, throw an error
                }
                else {
                    throw new AccountNotLinkedError("The account is already associated with another user", { provider: account.provider });
                }
            }
            else {
                // If the account does not exist, check if the user already has a passkey account
                const userAccounts = (await adapter.listLinkedAccounts(user.id)) ?? [];
                const passkeyAccount = userAccounts.find((a) => a.type === "passkey");
                // If yes, throw an error since a user can have at most one passkey account
                if (passkeyAccount) {
                    throw new AccountNotLinkedError("The user already has a passkey account", { provider: account.provider });
                }
                else {
                    // If not, link the account to the user
        ...
        if (userByAccount) {
            // If the account exists, sign the user in
            session = useJwtSession
                ? {}
                : await createSession({
                    sessionToken: generateSessionToken(),
            ...
            return { session, user: userByAccount, isNewUser: false };
        }
        else {
            // If the account does not exist, create a new user and sign them up
            const { id: _, ...newUser } = { ...profile, emailVerified: null };
            user = await adapter.createUser(newUser);
            await events.createUser?.({ user });
            await adapter.linkAccount({ ...account, userId: user.id });
            await events.linkAccount?.({ user, account, profile });
            session = useJwtSession
