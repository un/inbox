import { navigateTo, defineNuxtPlugin, useToast } from '#imports';
...
export const errorHandler: TRPCLink<TrpcCommandRouter> = () => {
  return ({ next, op }) => {
...
export default defineNuxtPlugin(() => {
  const trpcPlatformClient = createTRPCNuxtClient<TrpcCommandRouter>({
    ...
    links: [
      ...
      httpBatchLink({
        ...
        maxURLLength: 2083,
        fetch: (input, init) =>
          fetch(input, { credentials: 'include', ...init, signal: undefined })
      })
    ]
  });
